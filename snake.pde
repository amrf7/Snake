//Clase para el objeto snake
class snake{
  int[] x = new int[10], y = new int[10]; //Arreglos para las posiciones (x,y) de los circulos qie dibujan a la serpiente
  int x0, y0; //Variables para las velocidades en x o y
  int pos = 0, puntaje = 0; //Variables iniciales del puntaje y la posición de la serpiente 
  int vel; //Variable para la velocidad de la serpeinte
  
  //Constructor de la clase
  snake(){
    //Inicialización de las posiciones (x,y) de la serpiente
    x[0]=width/2;
    y[0]=height/2;
    
    //Ciclo para inicializar el resto de indices de los arreglos
  ...
  //Método para mostrar a la serpiente
  void mostrar(){
    ...
    //Ciclo para mostrar todos los elementos y propiedades de la serpiente según se mueva
    for(int i = 0; i < x.length; i++){
      //Color, tamaño y posición de la serpiente
      fill(x[i]/2,255,255);
      ellipse(x[i],y[i],20,20);
      if(y0!=0){ //Condición para mostrar los ojitos de la serpiente cuando va en vertical
        fill(255);
        ...
        ellipse(x[0]+5,y[0],4,4);
      } else { //Condición para mostrar los ojitos de la serpiente cuando va en horixontal 
        fill(255);
